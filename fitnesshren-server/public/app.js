  ...
  <div class="flex flex-row flex-1 clear">
    ...
    <div class="flex flex-column col col-9">
      ...
      <form class="flex flex-row flex-space-between" id="send-message">
        ...
        <input type="text" name="info" class="flex flex-1"> 
    ...
    chat.insertAdjacentHTML( 'beforeend', `<div class="message flex flex-row">
      <div class="message-wrapper">
        <p class="message-header">
          ...
          <span class="sent-date font-300">${moment(message.date).format('MMM Do, hh:mm:ss')}</span>
        </p>
        <p class="message-content font-300">${type}</p>
        <p class="message-content font-300">${info}</p>
      </div>
    </div>`);
...
const showChat = async () => {
  ...
  const messages = await client.service('history').find({
    query: {
      $sort: { createdAt: -1 },
      $limit: 25	
    }
...
document.addEventListener('submit', async ev => {
  if(ev.target.id === 'send-message') {
    ...
    await client.service('history').create({
      type: type.value,
	  info: info.value	
    });
...
client.service('history').on('created', addMessage);
